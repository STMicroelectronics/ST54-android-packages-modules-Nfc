...
void HalAidlBinderDied(void* /* cookie */) {
  LOG(ERROR) << StringPrintf(
      "%s: INfc aidl hal died, exiting procces to restart", __func__);
...
NfcAdaptation::NfcAdaptation() {
  // include VERBOSE logs by default
...
void NfcAdaptation::Initialize() {
  ...
  LOG(INFO) << StringPrintf(
      "%s; ver=%s nfa=%s st=25Q2-BP2A-20250405-Gen-25W14p0", func, "AOSP",
      "ST");
  ...
  if (mHal == nullptr) {
    ...
    if (mAidlHal != nullptr) {
      ...
      if (mAidlHalVer <= 1) {
        ...
        LOG(INFO) << StringPrintf("%s: sVndExtnsPresent: %s", func,
                                  sVndExtnsPresent ? "true" : "false");
      } else {
        LOG(INFO) << StringPrintf("%s: sNfcVendorExtn not initialized", func);
        LOG(INFO) << StringPrintf("%s: get_vsr_api_level: %d", func,
                                  get_vsr_api_level());
      }
    }
...
bool ThreadCondVar::wait(long millisec) {
  ...
  if ((waitResult != 0) && (waitResult != ETIMEDOUT))
    LOG(ERROR) << StringPrintf(
        "ThreadCondVar::wait: fail timed wait; error=0x%X", waitResult);
