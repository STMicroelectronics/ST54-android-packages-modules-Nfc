...
std::string searchLibPath(std::string file_name) {
  const std::vector<std::string> search_path = {"/system/lib/",
                                                "/system/lib64/"};
...
// 3. If none of 1,2 is defined, search a default file name
// "libnfc_vendor_extn.so".
...
void phNfcExtn_LibClose() {
  ...
  if (p_oem_extn_handle != NULL) {
    LOG(DEBUG) << StringPrintf("%s: Closing %s!!", __func__,
                               mLibPathName.c_str());
