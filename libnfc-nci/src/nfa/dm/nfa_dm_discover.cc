...
tNFA_DM_DISC_TECH_PROTO_MASK nfa_dm_change_listen_mask(
  ...
  if (mute_tech_route_option) {
    /* Check listening tech */
    LOG(VERBOSE) << StringPrintf("%s: listen tech will be changed to 0x%x",
                                 __func__, change_listen_mask);
    ...
    LOG(VERBOSE) << StringPrintf("%s: listen tech will set to 0x%x", __func__,
                                 (dm_disc_mask & NFA_DM_DISC_MASK_LISTEN));
  } else {
    LOG(VERBOSE) << StringPrintf(
        "%s:  listen tech=0x%x, unset tech muted in RT, added to discovery",
        __func__, change_listen_mask);
    if ((change_listen_mask & NFA_TECHNOLOGY_MASK_A) == 0) {
      dm_disc_mask |= (NFA_DM_DISC_MASK_LA_T1T | NFA_DM_DISC_MASK_LA_T2T |
                       NFA_DM_DISC_MASK_LA_ISO_DEP);
    }
    if ((change_listen_mask & NFA_TECHNOLOGY_MASK_B) == 0) {
      dm_disc_mask |= NFA_DM_DISC_MASK_LB_ISO_DEP;
    }
    if ((change_listen_mask & NFA_TECHNOLOGY_MASK_F) == 0) {
      dm_disc_mask |= (NFA_DM_DISC_MASK_LF_T3T);
    }
    LOG(VERBOSE) << StringPrintf("%s: listen tech will set to 0x%x", __func__,
                                 (dm_disc_mask & NFA_DM_DISC_MASK_LISTEN));
...
tNFA_DM_DISC_TECH_PROTO_MASK nfa_dm_change_poll_mask(
    tNFA_DM_DISC_TECH_PROTO_MASK dm_disc_mask,
    tNFA_TECHNOLOGY_MASK change_poll_mask) {
  if (change_poll_mask & NFA_TECHNOLOGY_MASK_A) {
    dm_disc_mask |= (NFA_DM_DISC_MASK_PA_T1T | NFA_DM_DISC_MASK_PA_T2T |
                     NFA_DM_DISC_MASK_PA_ISO_DEP | NFA_DM_DISC_MASK_PA_NFC_DEP |
                     NFA_DM_DISC_MASK_P_LEGACY);
  }
  if (change_poll_mask & NFA_TECHNOLOGY_MASK_B)
    dm_disc_mask |= NFA_DM_DISC_MASK_PB_ISO_DEP;

  if (change_poll_mask & NFA_TECHNOLOGY_MASK_F)
    dm_disc_mask |= (NFA_DM_DISC_MASK_PF_T3T | NFA_DM_DISC_MASK_PF_NFC_DEP);

  if (change_poll_mask & NFA_TECHNOLOGY_MASK_V)
    dm_disc_mask |= NFA_DM_DISC_MASK_P_T5T;

  if (change_poll_mask & NFA_TECHNOLOGY_MASK_B_PRIME)
    dm_disc_mask |= NFA_DM_DISC_MASK_P_B_PRIME;

  if (change_poll_mask & NFA_TECHNOLOGY_MASK_KOVIO)
    dm_disc_mask |= NFA_DM_DISC_MASK_P_KOVIO;

  LOG(VERBOSE) << StringPrintf("%s: poll tech mask will set to 0x%x", __func__,
                               (dm_disc_mask & NFA_DM_DISC_MASK_POLL));
...
static uint8_t nfa_dm_get_rf_discover_config(
  ...
  if (nfa_dm_cb.flags & NFA_DM_FLAGS_LISTEN_TECH_CHANGED) {
    LOG(VERBOSE) << StringPrintf("%s: listen tech will be changed to 0x%x",
                                 __func__, nfa_dm_cb.change_listen_mask);
    dm_disc_mask =
        nfa_dm_change_listen_mask(dm_disc_mask, nfa_dm_cb.change_listen_mask);
  } else if (dm_disc_listen_mask_dfl != 0) {
    LOG(VERBOSE) << StringPrintf("%s: default listen tech 0x%x will be used",
                                 __func__, dm_disc_listen_mask_dfl);
    dm_disc_mask =
  ...
  if (nfa_dm_cb.flags & NFA_DM_FLAGS_POLL_TECH_CHANGED) {
    /* Check polling tech */
    LOG(VERBOSE) << StringPrintf("%s: poll tech will be changed to 0x%x",
                                 __func__, nfa_dm_cb.change_poll_mask);
    dm_disc_mask &= NFA_DM_DISC_MASK_LISTEN;
    dm_disc_mask =
        nfa_dm_change_poll_mask(dm_disc_mask, nfa_dm_cb.change_poll_mask);
  } else if (dm_disc_poll_mask_dfl != 0) {
    LOG(VERBOSE) << StringPrintf("%s: default poll tech 0x%x will be used",
                                 __func__, dm_disc_poll_mask_dfl);
    dm_disc_mask = nfa_dm_change_poll_mask(dm_disc_mask, dm_disc_poll_mask_dfl);
  }
  ...
  if (dm_disc_mask & NFA_DM_DISC_MASK_P_B_PRIME) {
    if (NFC_DISCOVERY_TYPE_POLL_B_PRIME != NFA_PROTOCOL_INVALID) {
      ...
      LOG(ERROR) << StringPrintf("%s: Unsupported type POLL_B_PRIME!",
                                 __func__);
    }
  }
  ...
  if (dm_disc_mask & NFA_DM_DISC_MASK_P_KOVIO) {
    if (NFC_DISCOVERY_TYPE_POLL_KOVIO != NFA_PROTOCOL_INVALID) {
      ...
      LOG(ERROR) << StringPrintf("%s: Unsupported type POLL_KOVIO!", __func__);
    }
  }
  ...
  if (dm_disc_mask & NFA_DM_DISC_MASK_L_B_PRIME) {
    if (NFC_DISCOVERY_TYPE_LISTEN_B_PRIME != NFA_PROTOCOL_INVALID) {
      ...
      LOG(ERROR) << StringPrintf("%s: Unsupported type LISTEN_B_PRIME!",
                                 __func__);
    }
  }
...
static tNFA_STATUS nfa_dm_set_rf_listen_mode_config(
  ...
  LOG(VERBOSE) << StringPrintf("%s: tech_proto_mask = 0x%08X", __func__,
                               tech_proto_mask);
...
static void nfa_dm_set_rf_listen_mode_raw_config(
  ...
  *p_disc_mask = disc_mask;
  ...
  LOG(VERBOSE) << StringPrintf("%s: disc_mask = 0x%x", __func__, disc_mask);
}
...
static tNFA_DM_DISC_TECH_PROTO_MASK nfa_dm_disc_get_disc_mask(
  ...
  if (NFC_DISCOVERY_TYPE_POLL_A == tech_n_mode) {
    ...
    if (tech_n_mode == NFC_PROTOCOL_MIFARE) {
  ...
  LOG(VERBOSE) << StringPrintf(
      "%s: tech_n_mode=0x%X, protocol=0x%X, "
      "disc_mask=0x%X",
      __func__, tech_n_mode, protocol, disc_mask);
  return (disc_mask);
}
...
static void nfa_dm_disc_discovery_cback(tNFC_DISCOVER_EVT event,
  ...
  switch (event) {
    ...
    case NFC_DEACTIVATE_DEVT:
      ...
      break;
    case NFC_INTF_EXT_START_DEVT:
      dm_disc_event = NFA_DM_RF_INTF_EXT_START_RSP;
      break;
...
static tNFC_STATUS nfa_dm_disc_force_to_idle(void) {
  ...
  LOG(VERBOSE) << StringPrintf("%s: disc_flags = 0x%x", __func__,
...
void nfa_dm_start_rf_discover(void) {
  ...
  if (nfa_dm_cb.disc_cb.excl_disc_entry.in_use) {
    ...
    for (xx = 0; xx < NFA_DM_DISC_NUM_ENTRIES; xx++) {
      ...
      */
      ...
      LOG(VERBOSE) << StringPrintf(
          "%s:  nfa_dm_cb.disc_cb.entry[%d].selected_disc_mask = 0x%x",
          __func__, xx, nfa_dm_cb.disc_cb.entry[xx].selected_disc_mask);
  ...
  LOG(VERBOSE) << StringPrintf("%s: dm_disc_mask = 0x%x", __func__,
                               dm_disc_mask);
...
/*******************************************************************************
**
** Function         nfa_dm_start_rf_intf_ext
**
** Description      Send RF interface extension to NFCC
**
** Returns          void
**
*******************************************************************************/
void nfa_dm_start_rf_intf_ext(tNFA_INTF_EXT_TYPE extension_id, uint8_t* p_data,
                              uint8_t len) {
  tNFA_DM_DISC_RF_EXT_PARAMS start_ext_params;

  LOG(VERBOSE) << StringPrintf(
      "%s:  rf_ext_id=0x%X, Start Parameter length=0x%X", __func__,
      extension_id, len);

  if ((extension_id != NCI_INTF_EXT_WLCP_NON_AUTO) &&
      (extension_id != NCI_INTF_EXT_WLCP_SEMI_AUTO)) {
    LOG(ERROR) << StringPrintf("%s:  Not supported RF Interface Extension",
                               __func__);
    return;
  }
  ...
  if ((nfa_dm_cb.disc_cb.disc_state == NFA_DM_RFST_POLL_ACTIVE) ||
    ...
    /* protocol stack only implementing RF interface extension for WLC so far */
    nfa_dm_cb.flags |= NFA_DM_FLAGS_ENABLE_WLCP_PEND;

    /* state is OK: notify the status when the response is received from NFCC */
    start_ext_params.rf_intf_ext_id = extension_id;
    start_ext_params.p_cmd_params = p_data;
    start_ext_params.cmd_params_len = len;

    nfa_dm_cb.disc_cb.disc_flags |= NFA_DM_DISC_FLAGS_NOTIFY;
    tNFA_DM_RF_DISC_DATA nfa_dm_rf_ext_data;
    nfa_dm_rf_ext_data.handle_ext = start_ext_params;
    nfa_dm_disc_sm_execute(NFA_DM_RF_INTF_EXT_START_CMD, &nfa_dm_rf_ext_data);
  } else {
    nfa_dm_cb.flags &= ~NFA_DM_FLAGS_ENABLE_WLCP_PEND;
...
void nfa_dm_start_wireless_power_transfer(uint8_t power_adj_req,
  ...
  if ((nfa_dm_cb.disc_cb.disc_state == NFA_DM_RFST_POLL_ACTIVE) &&
    ...
    LOG(VERBOSE) << StringPrintf("%s:  wlc_cback_data.status=0x%X", __func__,
...
bool nfa_dm_disc_handle_kovio_activation(tNFC_DISCOVER* p_data,
  ...
  if (nfa_dm_cb.disc_cb.kovio_tle.in_use) {
    ...
    if ((nfa_dm_cb.activated_nfcid_len !=
      ...
      LOG(VERBOSE) << StringPrintf("%s: new Kovio tag is detected", __func__);
...
static tNFA_STATUS nfa_dm_disc_notify_activation(tNFC_DISCOVER* p_data) {
  ...
  LOG(VERBOSE) << StringPrintf("%s: tech_n_mode=0x%X, proto=0x%X", __func__,
  ...
  if (protocol == NFC_PROTOCOL_NFC_DEP &&
    ...
    if (appl_dta_mode_flag == 1 && tech_n_mode == NFC_DISCOVERY_TYPE_LISTEN_A) {
      LOG(VERBOSE) << StringPrintf(
          "%s: DTA Mode Enabled : NFC-A Passive Listen Mode", __func__);
    }
  }
  ...
  if (xx < NFA_DM_DISC_NUM_ENTRIES) {
    ...
    LOG(VERBOSE) << StringPrintf(
        "%s: activated_protocol=0x%x, activated_handle=0x%x", __func__,
...
static void nfa_dm_disc_notify_deactivation(tNFA_DM_RF_DISC_SM_EVENT sm_event,
  ...
  LOG(VERBOSE) << StringPrintf("%s: activated_handle=%d", __func__,
                               nfa_dm_cb.disc_cb.activated_handle);
...
void nfa_dm_disc_new_state(tNFA_DM_RF_DISC_STATE new_state) {
  ...
  LOG(VERBOSE) << StringPrintf(
      "%s: old_state=%s (%d), new_state=%s (%d) "
      "disc_flags=0x%x",
      __func__, nfa_dm_disc_state_2_str(nfa_dm_cb.disc_cb.disc_state).c_str(),
  ...
  switch (event) {
    ...
    case NFA_DM_RF_DISCOVER_RSP:
      ...
      if (p_data->nfc_discover.status == NFC_STATUS_OK) {
        LOG(DEBUG) << StringPrintf("%s; polling loop started correclty",
                                   __func__);
    ...
    case NFA_DM_RF_DEACTIVATE_RSP:
      ...
      if (p_data->nfc_discover.status == NFC_STATUS_OK) {
        ...
        if (!(nfa_dm_cb.disc_cb.disc_flags & NFA_DM_DISC_FLAGS_W4_NTF)) {
          ...
          nfa_dm_start_rf_discover();
          polling_start_cnt++;
          LOG(DEBUG) << StringPrintf("%s; Starting polling loop ", __func__);
        }
...
static void nfa_dm_disc_sm_discovery(tNFA_DM_RF_DISC_SM_EVENT event,
  ...
  switch (event) {
    ...
    case NFA_DM_RF_INTF_ACTIVATED_NTF:
      if (nfa_dm_cb.disc_cb.disc_flags & NFA_DM_DISC_FLAGS_W4_RSP) {
        LOG(VERBOSE) << StringPrintf(
            "%s: RF Activated while waiting for deactivation RSP", __func__);
        ...
        if (nfa_dm_disc_notify_activation(&(p_data->nfc_discover)) ==
            NFA_STATUS_FAILED) {
          LOG(VERBOSE) << StringPrintf(
              "%s: Not matched, restart discovery after receiving "
              "deactivate ntf",
              __func__);
    ...
    case NFA_DM_RF_INTF_ACTIVATED_NTF:
      ...
      */
      ...
      if (nfa_dm_disc_notify_activation(&(p_data->nfc_discover)) ==
          NFA_STATUS_FAILED) {
        LOG(VERBOSE) << StringPrintf(
            "%s: Not matched, restart discovery after receiving deactivate ntf",
            __func__);
...
static void nfa_dm_disc_sm_poll_active(tNFA_DM_RF_DISC_SM_EVENT event,
  ...
  switch (event) {
    ...
    case NFA_DM_RF_DEACTIVATE_NTF:
      ...
      if ((p_data->nfc_discover.deactivate.type == NFC_DEACTIVATE_TYPE_SLEEP) ||
        ...
        if ((nfa_dm_cb.disc_cb.disc_flags & NFA_DM_DISC_FLAGS_STOPPING) &&
            (!old_sleep_wakeup_flag)) {
          LOG(DEBUG) << StringPrintf(
              "%s:  Rx DEACT_NTF(SLEEP) while stopping,"
              "resending DEACT_CMD(idle) now",
              __func__);
          /* stop discovery */
          NFC_Deactivate(NFA_DEACTIVATE_TYPE_IDLE);
        }
        ...
        LOG(VERBOSE) << __func__
                     << StringPrintf(
                            "%s: NFA_DM_RF_DEACTIVATE_NTF to discovery",
                            __func__);
        if (p_data->nfc_discover.deactivate.reason ==
            NFC_DEACTIVATE_REASON_DH_REQ_FAILED) {
    ...
    case NFA_DM_RF_INTF_EXT_START_CMD:
      /* if no deactivation ongoing */
      if (!(nfa_dm_cb.disc_cb.disc_flags & NFA_DM_DISC_FLAGS_W4_RSP)) {
        NFC_StartRfIntfExtension(p_data->handle_ext.rf_intf_ext_id,
                                 p_data->handle_ext.p_cmd_params,
                                 p_data->handle_ext.cmd_params_len);

      } else {
        LOG(VERBOSE) << StringPrintf(
            "%s:  Fail to start "
            "RF extension, deactivation ongoing",
            __func__);
        if (nfa_dm_cb.flags & NFA_DM_FLAGS_ENABLE_WLCP_PEND) {
          nfa_dm_cb.flags &= ~NFA_DM_FLAGS_ENABLE_WLCP_PEND;
          ...
          nfa_dm_disc_conn_event_notify(NFA_RF_INTF_EXT_RESULT_EVT,
                                        NFA_STATUS_FAILED);
        }
      }
      break;

    case NFA_DM_RF_INTF_EXT_START_RSP:
      if (nfa_dm_cb.disc_cb.disc_flags & NFA_DM_DISC_FLAGS_NOTIFY) {
        nfa_dm_cb.disc_cb.disc_flags &= ~NFA_DM_DISC_FLAGS_NOTIFY;

        /* notify application status of start */
        if (nfa_dm_cb.flags & NFA_DM_FLAGS_ENABLE_WLCP_PEND) {
          nfa_dm_cb.flags &= ~NFA_DM_FLAGS_ENABLE_WLCP_PEND;
          ...
          if (p_data->nfc_discover.status == NFA_STATUS_OK) {
            nfa_dm_cb.flags |= NFA_DM_FLAGS_RF_EXT_ACTIVE;
          }
    ...
    case NFA_DM_RF_INTF_EXT_STOP_CMD:
      break;
    ...
    case NFA_DM_WPT_START_RSP:
      if (nfa_dm_cb.disc_cb.disc_flags & NFA_DM_DISC_FLAGS_NOTIFY) {
        ...
        if (p_data->nfc_discover.status == NFC_STATUS_OK) {
          wlc_cback_data.status = NFA_STATUS_OK;
          nfa_wlc_cb.flags |= NFA_WLC_FLAGS_WPT_NTF_PENDING;
          LOG(VERBOSE) << StringPrintf("%s:  WPT started", __func__);
        } else {
          wlc_cback_data.status = NFA_STATUS_FAILED;
        }
        nfa_wlc_event_notify(NFA_WLC_START_WPT_RESULT_EVT, &wlc_cback_data);
  ...
  switch (event) {
    ...
    case NFA_DM_RF_DEACTIVATE_NTF:
      ...
      if (nfa_dm_cb.disc_cb.disc_flags & NFA_DM_DISC_FLAGS_W4_RSP) {
        ...
        /* notify deactivation after receiving deactivate RSP */
        LOG(VERBOSE) << StringPrintf(
            "%s:  Rx deactivate NTF while waiting for deactivate RSP",
            __func__);
        break;
      }
  ...
  switch (event) {
    ...
    case NFA_DM_RF_DEACTIVATE_NTF:
      ...
      if (p_data->nfc_discover.deactivate.type == NFC_DEACTIVATE_TYPE_IDLE) {
        ...
        LOG(ERROR) << StringPrintf("%s: Unexpected deactivation type",
                                   __func__);
        nfa_dm_disc_new_state(NFA_DM_RFST_IDLE);
        nfa_dm_start_rf_discover();
      }
...
static void nfa_dm_disc_sm_lp_listen(tNFA_DM_RF_DISC_SM_EVENT event,
  ...
  switch (event) {
    case NFA_DM_RF_INTF_ACTIVATED_NTF:
      ...
      if (nfa_dm_disc_notify_activation(&(p_data->nfc_discover)) ==
          NFA_STATUS_FAILED) {
        LOG(VERBOSE) << StringPrintf("%s: Not matched, unexpected activation",
                                     __func__);
      }
      break;
...
void nfa_dm_disc_sm_execute(tNFA_DM_RF_DISC_SM_EVENT event,
                            tNFA_DM_RF_DISC_DATA* p_data) {
  LOG(VERBOSE) << StringPrintf(
      "%s: state=%s (%d), event=%s(%d) disc_flags: "
      "0x%x",
      __func__, nfa_dm_disc_state_2_str(nfa_dm_cb.disc_cb.disc_state).c_str(),
...
void nfa_dm_delete_rf_discover(tNFA_HANDLE handle) {
  ...
  if (handle < NFA_DM_DISC_NUM_ENTRIES) {
    ...
    LOG(ERROR) << StringPrintf("%s: Invalid discovery handle", __func__);
  }
}
...
void nfa_dm_rf_discover_select(uint8_t rf_disc_id, tNFA_NFC_PROTOCOL protocol,
  ...
  LOG(VERBOSE) << StringPrintf(
      "%s: rf_disc_id=0x%X, protocol=0x%X, rf_interface=0x%X", __func__,
...
bool nfa_dm_rf_removal_detection(uint8_t waiting_time) {
  ...
  LOG(VERBOSE) << StringPrintf("%s:  waiting_time=0x%X", __func__,
                               waiting_time);
  ...
  if (nfa_dm_cb.disc_cb.disc_state == NFA_DM_RFST_IDLE) {
    ...
    if (deactivate_type == NFA_DEACTIVATE_TYPE_DISCOVERY) {
      if (nfa_dm_cb.disc_cb.kovio_tle.in_use) {
        ...
        LOG(VERBOSE) << StringPrintf("%s: already in discovery state",
                                     __func__);
        return NFA_STATUS_FAILED;
      }
...
static std::string nfa_dm_disc_event_2_str(uint8_t event) {
  switch (event) {
    ...
    case NFA_DM_RF_INTF_EXT_START_CMD:
      return "INTF_EXT_START_CMD";
    case NFA_DM_RF_INTF_EXT_START_RSP:
      return "INTF_EXT_START_RSP";
    case NFA_DM_RF_INTF_EXT_STOP_CMD:
...
/*******************************************************************************
**
** Function         nfa_dm_get_tech_route_block
**
** Description      Retrieves which tech shall be muted
**
** Returns
**
*******************************************************************************/
void nfa_dm_get_tech_route_block(uint8_t* listen_techmask, bool* enable) {
  if ((nfa_dm_cb.flags & NFA_DM_FLAGS_LISTEN_TECH_CHANGED)) {
    *enable = true;
    *listen_techmask = nfa_dm_cb.change_listen_mask;
  } else if (dm_disc_listen_mask_dfl != 0) {
    *enable = true;
    *listen_techmask = dm_disc_listen_mask_dfl;
  } else {
    *enable = false;
    *listen_techmask = 0;
  }
  ...
  if (*enable) {
    LOG(VERBOSE) << StringPrintf("%s:  change_listen_mask=0x%x, ", __func__,
                                 *listen_techmask);
  }
}
