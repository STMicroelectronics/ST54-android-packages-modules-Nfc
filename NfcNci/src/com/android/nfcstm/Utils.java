...
public final class Utils {
    ...
    public static void dumpDebugIntentFilter(
        ...
        for (int i = 0; i < intentFilter.countDataSchemeSpecificParts(); i++) {
            dumpDebugPatternMatcher(
                    intentFilter.getDataSchemeSpecificPart(i),
                    proto,
                    IntentFilterProto.DATA_SCHEME_SPECS);
        }
        for (int i = 0; i < intentFilter.countDataAuthorities(); i++) {
            dumpDebugAuthorityEntry(
                    intentFilter.getDataAuthority(i), proto, IntentFilterProto.DATA_AUTHORITIES);
        }
        for (int i = 0; i < intentFilter.countDataPaths(); i++) {
            dumpDebugPatternMatcher(
                    intentFilter.getDataPath(i), proto, IntentFilterProto.DATA_PATHS);
        }
     ...
     */
    private static void dumpDebugPatternMatcher(
            @NonNull PatternMatcher patternMatcher,
    ...
    private static void dumpDebugAuthorityEntry(
            @NonNull IntentFilter.AuthorityEntry authorityEntry,
    ...
    public static boolean hasCeServicesWithValidPermissions(
        ...
        try {
            ...
            return Arrays.stream(packageInfo.services)
                    .map(serviceInfo -> serviceInfo.permission)
            ...
            Log.e(
                    TAG,
                    "hasCeServicesWithValidPermissions: Could not create user package context"
                            + packageInfo);
            return false;
        }
    }
