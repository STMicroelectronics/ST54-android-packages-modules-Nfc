...
import static com.android.nfcstm.ScreenStateHelper.SCREEN_STATE_ON_LOCKED;
import static com.android.nfcstm.ScreenStateHelper.SCREEN_STATE_ON_UNLOCKED;
...
public class NfcService implements DeviceHostListener, ForegroundUtils.Callback {
    ...
    @VisibleForTesting CardEmulationManager mCardEmulationManager;
    ...
    private final Set<INfcWlcStateListener> mWlcStateListener =
            Collections.synchronizedSet(new HashSet<>());
    @Nullable private final StatsdUtils mStatsdUtils;
    ...
    /**
     * Notifies Removal Detection procedure completed, endpoint deactivation reason returned by NFCC
     */
    ...
    public void onRemoteFieldDeactivated() {
        ...
        if (Flags.coalesceRfEvents()) {
            mHandler.sendMessageDelayed(
                    mHandler.obtainMessage(MSG_RF_FIELD_DEACTIVATED), RF_COALESCING_WINDOW);
        } else {
    ...
    final Runnable mPollingLoopsDetectedRunnable =
            new Runnable() {
                public void run() {
    ...
    boolean shouldEnableNfc() {
        return getNfcOnSetting()
                && !mNfcInjector.isSatelliteModeOn()
                && !isNfcUserRestricted()
                && allowOemEnable();
    }
    ...
    public NfcService(Application nfcApplication, NfcInjector nfcInjector) {
        ...
        mIsSecureNfcEnabled =
                mPrefs.getBoolean(
                                PREF_SECURE_NFC_ON, mDeviceConfigFacade.getDefaultSecureNfcState())
                        && mIsSecureNfcCapable;
        ...
        mTagAppBlockListHash =
                mPrefs.getInt(PREF_TAG_APP_BLOCK_LIST_HASH, PREF_TAG_APP_BLOCK_LIST_HASH_DEFAULT);

        // Get default blocked package list from resource file
        TAG_APP_BLOCKLIST.addAll(
                Arrays.asList(
                        mContext.getResources()
                                .getStringArray(R.array.tag_intent_blocked_app_list)));
                ...
                new BroadcastReceiver() {
                    ...
                    public void onReceive(Context context, Intent intent) {
                        ...
                        if (isTaskBootCompleted()) {
                            ...
                            Log.i(
                                    TAG,
                                    "restriction change detected - skip NFC init is not completed");
                        }
                    }
                },
    ...
    private void initTagAppPrefList() {
        ...
        if (force) {
            ...
            mPrefsEditor.putInt(PREF_TAG_APP_BLOCK_LIST_HASH, mTagAppBlockListHash);
            mPrefsEditor.apply();
        }
        ...
        try {
            for (Integer userId : getEnabledUserIds()) {
                ...
                for (String pkg : TAG_APP_BLOCKLIST) {
                    if ((force || !map.containsKey(pkg)) && isPackageInstalled(pkg, userId)) {
                        map.put(pkg, false);
                        changed = true;
                    }
                }
    ...
    void initSoundPoolIfNeededAndPlaySound(Runnable playSoundRunnable) {
        if (mSoundPool == null) {
            ...
            Log.d(
                    TAG,
                    "initSoundPoolIfNeededAndPlaySound: Sound pool is already loaded, "
                            + "playing sound");
            playSoundRunnable.run();
        }
    }
    ...
    class EnableDisableTask extends AsyncTask<Integer, Void, Boolean> {
        ...
        protected void onPostExecute(Boolean result) {
            ...
            if (mNfcOemExtensionCallback != null) {
                try {
                    if (action == TASK_BOOT)
                        mNfcOemExtensionCallback.onBootFinished(
                                result ? STATUS_OK : STATUS_UNKNOWN_ERROR);
                    else if (action == TASK_ENABLE)
                        mNfcOemExtensionCallback.onEnableFinished(
                                result ? STATUS_OK : STATUS_UNKNOWN_ERROR);
                    else if (action == TASK_DISABLE)
                        mNfcOemExtensionCallback.onDisableFinished(
                                result ? STATUS_OK : STATUS_UNKNOWN_ERROR);
                } catch (RemoteException remoteException) {
        ...
        void onOemPreExecute() {
            if (mNfcOemExtensionCallback != null) {
                try {
                    if (action == TASK_BOOT) mNfcOemExtensionCallback.onBootStarted();
                    else if (action == TASK_ENABLE) mNfcOemExtensionCallback.onEnableStarted();
                    else if (action == TASK_DISABLE) mNfcOemExtensionCallback.onDisableStarted();
                } catch (RemoteException remoteException) {
        ...
        boolean enableInternal() {
            ...
            if (mState == NfcAdapter.STATE_ON) {
                ...
                if (!isAlwaysOnInDefaultMode()) {
                    Log.i(
                            TAG,
                            "enableInternal: ControllerAlwaysOn Not In DEFAULT_MODE "
                                    + "- disableAlwaysOn!");
            ...
            if (mInProvisionMode) {
                ...
                mScreenState =
                        mScreenStateHelper.checkScreenState(mCheckDisplayStateForScreenState);
            }
        ...
        boolean disableAlwaysOnInternal() {
            if (mAlwaysOnState == NfcAdapter.STATE_OFF) {
                ...
                updateAlwaysOnState(NfcAdapter.STATE_OFF);
            } else if (mState == NfcAdapter.STATE_OFF || (mAlwaysOnState == NfcAdapter.STATE_ON)) {
                ...
                if (mAlwaysOnState != NfcAdapter.STATE_TURNING_ON) {
                    ...
                    if (mNfcOemExtensionCallback != null) {
                        try {
                            ...
                            Log.e(
                                    TAG,
                                    "updateState: Failed to invoke onStateUpdated "
                                            + "oem callback");
                        }
                    }
                }
    ...
    private void resetReaderModeParams() {
        synchronized (NfcService.this) {
            if (mPollingDisableDeathRecipients.size() == 0) {
                Log.d(
                        TAG,
                        "resetReaderModeParams: Disabling reader mode because app died"
                ...
                // ST21NFC(end)
                // listenTech is different from the default value, the stored listenTech will be
                // included.
                // When using enableReaderMode, change listenTech to default & restore to the
                // previous value.
                if (isNfcEnabled()) {
                    restoreSavedTech();
                }
    ...
    final class NfcAdapterService extends INfcAdapter.Stub {
        ...
        public boolean enable(String pkg) throws RemoteException {
            ...
            notifyOemLogEvent(
                    new OemLogItems.Builder(OemLogItems.LOG_ACTION_NFC_TOGGLE)
            ...
            synchronized (NfcService.this) {
                ...
                if (!NfcInjector.isPrivileged(callingUid)) {
                    ...
                    if (packageName == null) {
                        Log.e(
                                TAG,
                                "setObserveMode: no package name associated with "
        ...
        public int pausePolling(long timeoutInMs) {
            ...
            synchronized (NfcService.this) {
                ...
                if (timeoutInMs < 0 || timeoutInMs > this.getMaxPausePollingTimeoutMs()) {
                    throw new IllegalArgumentException("Invalid timeout " + timeoutInMs + " ms!");
                }
                mHandler.sendMessageDelayed(
        ...
        public void updateDiscoveryTechnology(
            ...
            boolean privilegedCaller =
                    NfcInjector.isPrivileged(callingUid)
            ...
            if (!privilegedCaller) {
                ...
                if ((pollTech & NfcAdapter.FLAG_SET_DEFAULT_TECH) != 0) {
                    if ((pollTech & NfcAdapter.FLAG_READER_KEEP) == 0
                        ...
                        Log.i(
                                TAG,
                                "updateDiscoveryTechnology: Default pollTech is set to 0x"
                ...
                if ((listenTech & NfcAdapter.FLAG_SET_DEFAULT_TECH) != 0) {
                    if ((listenTech & NfcAdapter.FLAG_LISTEN_KEEP) == 0
                        ...
                        Log.i(
                                TAG,
                                "updateDiscoveryTechnology: Default listenTech is set to 0x"
            ...
            synchronized (NfcService.this) {
                ...
                if (pollTech == NfcAdapter.FLAG_USE_ALL_TECH
                    ...
                    try {
                        ...
                        Log.e(
                                TAG,
                                "updateDiscoveryTechnology: Change Tech Binder was never "
                                        + "registered");
                    }
                    ...
                    try {
                        ...
                        if (android.nfc.Flags.nfcPersistLog()) {
                            mNfcEventLog.logEvent(
                                    NfcEventProto.EventType.newBuilder()
                                            .setDiscoveryTechnologyUpdate(
                                                    NfcEventProto.NfcDiscoveryTechnologyUpdate
                                                            .newBuilder()
                                                            .setAppInfo(
                                                                    NfcEventProto.NfcAppInfo
                                                                            .newBuilder()
                                                                            .setPackageName(
                                                                                    packageName)
                                                                            .setUid(callingUid)
            ...
            synchronized (NfcService.this) {
                ...
                if (flags != 0) {
                    try {
                        ...
                        // listenTech is different from the default value, the stored listenTech
                        // will be included.
                        // When using setReaderMode, change listenTech to default & restore to
                        // previous value.
                        if (isNfcEnabled()) {
                    ...
                    try {
                        ...
                        // listenTech is different from the default value, the stored listenTech
                        // will be included.
                        // When using enableReaderMode, change listenTech to default & restore to
                        // the previous value.
                        if (isNfcEnabled()) {
                            restoreSavedTech();
                        }
        ...
        public NfcAntennaInfo getNfcAntennaInfo() {
            ...
            if (positionX.length == 0 || positionY.length == 0) {
                positionX =
                        NfcProperties.info_antpos_X().stream()
                                .mapToInt(Integer::intValue)
                                .toArray();
                positionY =
                        NfcProperties.info_antpos_Y().stream()
                                .mapToInt(Integer::intValue)
                                .toArray();
            ...
            return new NfcAntennaInfo(width, height, isFoldable, availableNfcAntennas);
        }
        ...
        private int setTagAppPreferenceInternal(int userId, String pkg, boolean allow) {
            ...
            if (DBG) {
                Log.i(
                        TAG,
                        "setTagAppPreferenceInternal: UserId:"
                                + userId
                                + " pkg:"
                                + pkg
                                + ":"
                                ...
                                synchronized (NfcService.this) {
                                    if (isPowerSavingModeCmd(gid, oid, payload)) {
                                        ...
                                        NfcVendorNciResponse response =
                                                new NfcVendorNciResponse(
                                                        (byte) NCI_STATUS_OK,
                                                        NCI_GID_PROP,
                                                        NCI_MSG_PROP_ANDROID,
                                                        new byte[] {
                                                            (byte)
                                                                    NCI_PROP_ANDROID_QUERY_POWER_SAVING_STATUS_CMD,
                                                            0x00,
                                                            mIsPowerSavingModeEnabled
                                                                    ? (byte) 0x01
                                                                    : (byte) 0x00
                                                        });
        ...
        public Map<String, Integer> fetchActiveNfceeList() throws RemoteException {
            if (DBG) Log.i(TAG, "fetchActiveNfceeList");
        ...
        public void clearPreference() throws RemoteException {
            ...
            if (android.nfc.Flags.nfcPersistLog()) {
                mNfcEventLog.logEvent(
                        NfcEventProto.EventType.newBuilder()
                                .setClearPreference(
                                        NfcEventProto.NfcClearPreference.newBuilder().build())
                                .build());
            }
            mPrefsEditor.clear();
        ...
        public void checkFirmware() throws RemoteException {
            ...
            try {
                runTaskOnSingleThreadExecutor(checkFirmwareTask, CHECK_FIRMWARE_TIMEOUT_MS);
            } catch (TimeoutException e) {
        ...
        public void indicateDataMigration(boolean inProgress, String pkg) throws RemoteException {
            ...
            mNfcEventLog.logEvent(
                    NfcEventProto.EventType.newBuilder()
                            .setDataMigrationInProgress(
                                    NfcEventProto.NfcDataMigrationInProgress.newBuilder()
                                            .setAppInfo(
                                                    NfcEventProto.NfcAppInfo.newBuilder()
                                                            .setPackageName(pkg)
                                                            .setUid(Binder.getCallingUid())
        ...
        public long getMaxPausePollingTimeoutMs() {
            ...
            NfcPermissions.enforceAdminPermissions(mContext);
            int timeoutOverlay =
                    mContext.getResources().getInteger(R.integer.max_pause_polling_time_out_ms);
    ...
    /**
     * Stops the Presence check thread without calling Disconnect API and onTagDisconnect callback
     */
    ...
    public int commitRouting(boolean isOverrideOrRecover) {
        ...
        try {
            boolean success =
                    mCommitRoutingCountDownLatch.await(
                            WAIT_FOR_COMMIT_ROUTING_TIMEOUT_MS, TimeUnit.MILLISECONDS);
            if (!success) {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_COMMIT_ROUTING:
                    {
                        ...
                        synchronized (NfcService.this) {
                            if (isNfcDisabledOrDisabling()) {
                                Log.d(
                                        TAG,
                                        "handleMessage: Skip commit routing when NFCC is off "
                                                + "or turning off");
                    ...
                    if (mIsWlcEnabled) {
                        ...
                        if (!mNfcCharging.NfcChargingMode
                            ...
                            if (mNfcCharging.startNfcCharging(tag)) {
                                ...
                                if (DBG) {
                                    Log.d(TAG, "handleMessage: Nfc charging mode not detected");
                                }
                            }
                ...
                case MSG_RF_FIELD_ACTIVATED:
                    ...
                    notifyOemLogEvent(
                            new OemLogItems.Builder(OemLogItems.LOG_ACTION_RF_FIELD_STATE_CHANGED)
                                    .setRfFieldOnTime(Instant.now())
                                    .build());
                    if (mCardEmulationManager != null) {
                ...
                case MSG_APPLY_SCREEN_STATE:
                    ...
                    Log.d(
                            TAG,
                            "handleMessage: MSG_APPLY_SCREEN_STATE"
                    ...
                    try {
                        ...
                        if (mNfcUnlockManager.isLockscreenPollingEnabled()) {
                            ...
                            if (mIsQiCharging) {
                                screen_state_mask =
                                        mScreenStateHelper.adaptMaskForQiCharging(
                                                screen_state_mask);
                                Log.d(
                                        TAG,
            ...
            try {
                ...
                for (int userId : mNfcEventInstalledPackages.keySet()) {
                    ...
                    if (DBG) {
                        ...
                        Log.d(
                                TAG,
                                "sendOffHostTransactionEvent: queryBroadcastReceiversAsUser: "
                ...
                Log.e(
                        TAG,
                        "sendOffHostTransactionEvent: Incorrect format for Secure Element name"
                                + e);
            } catch (IllegalArgumentException e) {
    ...
    boolean checkAndHandleRemovalDetectionMode(boolean isDisconnectNeeded) {
        if (mAppInActivityDetectionTimer != null) {
            if (isPresenceCheckStopped()) {
                ...
                Log.d(
                        TAG,
                        "checkAndHandleRemovalDetectionMode: Clearing Removal "
    ...
    class AppInActivityHandlerTask extends TimerTask {
        public void run() {
            Log.d(
                    TAG,
                    "run: App Inactivity detected, Requesting to Start Removal "
                            + "Detection Procedure");
            if (isTagPresent()) {
                ...
                mHandler.post(
                        () ->
                                Toast.makeText(
                                                mContext,
                                                "No activity over reader mode, RF removal detection"
                                                        + " procedure started",
                                                Toast.LENGTH_LONG)
                                        .show());
            ...
            new BroadcastReceiver() {
                ...
                public void onReceive(Context context, Intent intent) {
                    ...
                    if (action.equals(Intent.ACTION_SCREEN_ON)
                        ...
                        mIsQiCharging = checkQiState();
                        if (mIsQiCharging) {
                            Log.d(TAG, "mReceiver - onReceive() -  Wireless charging started");
                            sendMessage(
                        ...
                        mIsQiCharging = false;
                        if (wasQi) {
                            Log.d(TAG, "mReceiver - onReceive() -  Wireless charging stopped");
                        ...
                        // If this is first unlock after upgrading to NFC stack that is direct boot
                        // aware,
                        // migrate over the data from CE directory to DE directory for access before
                        // user
                        // unlock in subsequent bootups.
                        if (!mPrefs.getBoolean(PREF_MIGRATE_TO_DE_COMPLETE, false)) {
                            Log.i(
                                    TAG,
                                    "mReceiver.onReceive: Migrating shared prefs to DE directory "
                                            + "from CE directory");
                            ...
                            SharedPreferences ceTagPreferences =
                                    ceContext.getSharedPreferences(
                                            PREF_TAG_APP_LIST, Context.MODE_PRIVATE);
                            Log.d(
                                    TAG,
                                    "mReceiver.onReceive: CE Shared Pref values: "
                                            + cePreferences.getAll()
                                            + ", "
                            ...
                            if (ceTagPreferences.getAll().isEmpty()) {
                                ...
                                if (!mContext.moveSharedPreferencesFrom(
                                        ceContext, PREF_TAG_APP_LIST)) {
                                    Log.e(
                                            TAG,
                                            "mReceiver.onReceive: Failed to migrate NFC Shared"
                            ...
                            // If the move is completed, refresh our reference to the shared
                            // preferences.
                            mPrefs = mContext.getSharedPreferences(PREF, Context.MODE_PRIVATE);
                            mPrefsEditor = mPrefs.edit();
                ...
                public void onReceive(Context context, Intent intent) {
                    ...
                    if (user == null) {
                        Log.d(
                                TAG,
                                intent.getAction()
                                        + "mManagedProfileReceiver.onReceive: broadcast without"
                                        + " EXTRA_USER");
                        return;
                    }
                    ...
                    if (action.equals(Intent.ACTION_MANAGED_PROFILE_ADDED)
                        ...
                        if (DBG) {
                            Log.d(
                                    TAG,
                                    action
                                            + "mManagedProfileReceiver.onReceive: UserId: "
                                            + user.getIdentifier());
                        }
    ...
    private void setPaymentForegroundPreference(int user) {
        ...
        try {
            ...
            Log.d(
                    TAG,
                    "setPaymentForegroundPreference: NFC_PAYMENT_FOREGROUND preference:"
    ...
    void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        if (mContext.checkCallingOrSelfPermission(android.Manifest.permission.DUMP)
            ...
            pw.println(
                    "Permission Denial: can't dump nfc from pid="
    ...
    public boolean setFirmwareExitFrameTable(List<ExitFrame> exitFrames, int timeoutMs) {
        ...
        boolean result =
                mDeviceHost.setFirmwareExitFrameTable(
                        exitFrames.toArray(ExitFrame[]::new), timeoutBytes);
