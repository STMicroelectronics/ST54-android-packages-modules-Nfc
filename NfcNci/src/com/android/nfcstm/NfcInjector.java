...
import com.android.nfcstm.cardemulation.util.StatsdUtilsContext;
...
/**
 * To be used for dependency injection (especially helps mocking static dependencies). TODO: Migrate
 * more classes to injector to resolve circular dependencies in the NFC stack.
 */
public class NfcInjector {
    private static final String TAG = "NfcInjector";
    private static final String APEX_NAME = "com.android.nfcstmservices";
    ...
    @Nullable private final StatsdUtilsContext mStatsdUtilsContext;
    ...
    public NfcInjector(@NonNull Context context, @NonNull Looper mainLooper) {
        ...
        mStatsdUtils =
                mFeatureFlags.statsdCeEventsFlag() ? new StatsdUtils(mStatsdUtilsContext) : null;
        mForegroundUtils =
    ...
    public CardEmulationManager getCardEmulationManager() {
        if (mCardEmulationManager == null) {
            mCardEmulationManager =
                    new CardEmulationManager(mContext, sInstance, mDeviceConfigFacade);
        }
    ...
    public boolean isSatelliteModeSensitive() {
        final String satelliteRadios =
                Settings.Global.getString(
                        mContext.getContentResolver(), Constants.SETTINGS_SATELLITE_MODE_RADIOS);
    ...
    public boolean isSatelliteModeOn() {
        ...
        return Settings.Global.getInt(
                        mContext.getContentResolver(), Constants.SETTINGS_SATELLITE_MODE_ENABLED, 0)
                == 1;
    }
