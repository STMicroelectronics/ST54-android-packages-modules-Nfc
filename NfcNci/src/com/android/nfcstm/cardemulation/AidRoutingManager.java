...
public class AidRoutingManager {
    ...
    private boolean isAidEntryUpdated(
            HashMap<String, Integer> currRouteForAid,
    ...
    private boolean checkUnrouteAid(
            HashMap<String, Integer> prevRouteForAid, HashMap<String, Integer> prevPowerForAid) {
    ...
    private boolean checkRoutingOptionChanged(
            int prevDefaultRoute, int prevDefaultIsoDepRoute, int prevDefaultOffHostRoute) {
    ...
    private void checkOffHostRouteToHost(HashMap<String, AidEntry> routeCache) {
        ...
        while (it.hasNext()) {
            ...
            boolean mustHostRoute =
                    aidEntry.unCheckedOffHostSE.stream()
                            .anyMatch(
                                    offHost ->
                                            mRoutingOptionManager.getRouteForSecureElement(offHost)
                                                    == mDefaultRoute);
            if (mustHostRoute) {
                if (DBG) {
                    Log.d(
                            TAG,
                            "checkOffHostRouteToHost: "
                                    + aid
                                    + " is route to host due to unchecked off host and "
                                    + "default route(0x"
                                    + Integer.toHexString(mDefaultRoute)
                                    + ") is same");
                }
            } else {
                if (DBG) {
    ...
    public int configureRouting(
            HashMap<String, AidEntry> aidMap, boolean force, boolean isOverrideOrRecover) {
        ...
        for (Map.Entry<String, AidEntry> aidEntry : aidMap.entrySet()) {
            ...
            if (!seList.contains(route)) seList.add(route);
        ...
        synchronized (mLock) {
            ...
            for (int index = 0; index < seList.size(); index++) {
                ...
                if (index != 0) {
                    if (DBG) {
                        Log.d(
                                TAG,
                                "configureRouting: AidRoutingTable is full, try to switch "
                                        + "mDefaultRoute to 0x"
                                        + Integer.toHexString(mDefaultRoute));
                    }
                }
                            ...
                            for (Map.Entry<String, Integer> aidEntry : mRouteForAid.entrySet()) {
                                ...
                                if (defaultRouteAid.startsWith(aid) && route != mDefaultRoute) {
                                    ...
                                    aidRoutingTableCache.put(
                                            defaultRouteAid, aidMap.get(defaultRouteAid));
                                }
                            }
                        }
                    }
                            ...
                            if (aid.endsWith("*")) {
                                if (mAidMatchingSupport == AID_MATCHING_EXACT_ONLY) {
                                    ...
                                    if (DBG) {
                                        Log.d(
                                                TAG,
                                                "configureRouting: Routing prefix AID "
                                                        + aid
                                                        + " to route "
                                                        + Integer.toString(route));
                                    }
                                    // Cut off '*' since controller anyway treats all AIDs as a
                                    // prefix
                                    aidRoutingTableCache.put(
                                            aid.substring(0, aid.length() - 1), aidMap.get(aid));
                                } else if (mAidMatchingSupport == AID_MATCHING_EXACT_OR_PREFIX
                                        || mAidMatchingSupport
                                                == AID_MATCHING_EXACT_OR_SUBSET_OR_PREFIX) {
                                    if (DBG) {
                                        Log.d(
                                                TAG,
                                                "configureRouting: Routing prefix AID "
                                ...
                                if (mAidMatchingSupport == AID_MATCHING_EXACT_ONLY) {
                                    ...
                                    Log.e(
                                            ...
                                            "configureRouting: Device does not support subset "
                                                    ...
                                                    + aid
                                                    + "] is registered");
                                } else if (mAidMatchingSupport
                                        == AID_MATCHING_EXACT_OR_SUBSET_OR_PREFIX) {
                                    if (DBG) {
                                        Log.d(
                                                TAG,
                                                "configureRouting: Routing subset AID "
                                                        + aid
                                                        + " to route "
                                                        + Integer.toString(route));
                                    }
                                ...
                                if (DBG) {
                                    Log.d(
                                            TAG,
                                            "configureRouting: Routing exact AID "
                                                    + aid
                                                    + " to route "
                                                    + Integer.toString(route));
                                }
                ...
                if (NfcService.getInstance().getNciVersion()
                    ...
                    if (mDefaultRoute == ROUTE_HOST) {
                        ...
                        default_route_power_state =
                                RegisteredAidCache.POWER_STATE_SWITCH_ON
            ...
            boolean isRoutingOptionUpdated =
                    checkRoutingOptionChanged(
                            prevDefaultRoute, prevDefaultIsoDepRoute, prevDefaultOffHostRoute);

            if (isPowerStateUpdated
                    || isRouteTableUpdated
                    || mIsUnrouteRequired
                    || isRoutingOptionUpdated
                    || force) {
                if (aidRouteResolved) {
                    ...
                    if (result != NfcOemExtension.COMMIT_ROUTING_STATUS_OK) {
                        NfcStatsLog.write(
                                ...
                                NfcStatsLog.NFC_ERROR_OCCURRED__TYPE__UNKNOWN,
                                0,
                                0);
                    ...
                    Log.e(
                            TAG,
                            "configureRouting: RoutingTable unchanged because it's full, "
                                    + "not updating");
                ...
                Log.e(
                        TAG,
                        "configureRouting: All AIDs routing to mDefaultRoute, RoutingTable"
        ...
        if (!mRoutingOptionManager.isRoutingTableOverrided()) {
            if (force || optionChanged) {
                ...
                NfcService.getInstance()
                        .setTechnologyABFRoute(mDefaultOffHostRoute, mDefaultFelicaRoute);
            }
        } else {
            Log.d(
                    TAG,
                    "sendRoutingTable: Routing table is override, "
