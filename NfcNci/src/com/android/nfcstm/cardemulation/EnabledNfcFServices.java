...
public class EnabledNfcFServices implements com.android.nfcstm.ForegroundUtils.Callback {
    ...
    void computeEnabledForegroundService() {
        ...
        synchronized (mLock) {
            if (mActivated) {
                Log.d(
                        TAG,
                        "computeEnabledForegroundService: configuration will be postponed "
                                + "until deactivation");
    ...
    public boolean registerEnabledForegroundService(ComponentName service, int callingUid) {
        ...
        synchronized (mLock) {
            ...
            NfcFServiceInfo serviceInfo = mNfcFServiceCache.getService(userId, service);
            if (serviceInfo == null) {
                return false;
            } else {
            ...
            if (service.equals(mForegroundRequested) && mForegroundUid == callingUid) {
                Log.e(
                        TAG,
                        "registerEnabledForegroundService: The service is already requested "
            ...
            if (mForegroundUtils.registerUidToBackgroundCallback(this, callingUid)) {
                ...
                Log.e(
                        TAG,
                        "registerEnabledForegroundService: Calling UID is not in "
    ...
    public boolean unregisteredEnabledForegroundService(int callingUid) {
        ...
        if (mForegroundUtils.isInForeground(callingUid)) {
            ...
            Log.e(
                    TAG,
                    "unregisterEnabledForegroundService: Calling UID is not in "
                            + "the foreground, ignorning!");
            return false;
        }
    }
