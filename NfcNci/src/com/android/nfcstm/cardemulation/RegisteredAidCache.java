...
import com.android.nfcstm.cardemulation.util.TelephonyUtils;
...
public class RegisteredAidCache {
    ...
    static final int POWER_STATE_ALL =
    ...
    static final int POWER_STATE_ALL_NCI_VERSION_1_0 =
            POWER_STATE_SWITCH_ON | POWER_STATE_SWITCH_OFF | POWER_STATE_BATTERY_OFF;
    ...
    public RegisteredAidCache(
        ...
        if (mSupportsPrefixes) {
            if (DBG) Log.d(TAG, "RegisteredAidCache: Controller supports AID prefix routing");
        }
        if (mSupportsSubset) {
            if (DBG) Log.d(TAG, "RegisteredAidCache: Controller supports AID subset routing");
        }
    }
    ...
    public AidResolveInfo resolveAid(String aid) {
        synchronized (mLock) {
            ...
            if (aid.length() < 10) {
                Log.e(TAG, "resolveAid: AID selected with fewer than 5 bytes.");
            ...
            if (mSupportsPrefixes || mSupportsSubset) {
                ...
                if (DBG) {
                    Log.d(
                            TAG,
                            "resolveAid: Finding AID registrations in range ["
                                    + shortestAidMatch
                                    + " - "
                                    + longestAidMatch
                                    + "]");
                ...
                for (Map.Entry<String, AidResolveInfo> entry : matchingAids.entrySet()) {
                    ...
                    String entryAid =
                            (isPrefix || isSubset)
                                    ...
                                    : entry.getKey(); // Cut off '*' if prefix
                    if (entryAid.equalsIgnoreCase(aid)
                            || (isPrefix && aid.startsWith(entryAid))
                        ...
                        if (entryResolveInfo.defaultService != null) {
                            if (resolveInfo.defaultService != null) {
                                ...
                                // default service.
                                Log.e(TAG, "resolveAid: Different defaults for conflicting AIDs!");
                            }
            ...
            if (DBG) Log.d(TAG, "resolveAid: Resolved to: " + resolveInfo);
            return resolveInfo;
        }
    }
    ...
    public boolean isDefaultServiceForAid(int userId, ComponentName service, String aid) {
        ...
        if (resolveInfo.defaultService != null) {
            ...
            Log.d(TAG, "isDefaultServiceForAid: Not Default Service: " + service.getClassName());
    ...
    ApduServiceInfo resolvePollingLoopFilterConflict(List<ApduServiceInfo> conflictingServices) {
        ...
        for (ApduServiceInfo serviceInfo : conflictingServices) {
            int userId = UserHandle.getUserHandleForUid(serviceInfo.getUid()).getIdentifier();
        ...
        if (mWalletRoleObserver.isWalletRoleFeatureEnabled()) {
            roleHolderServices.sort(
                    (o1, o2) ->
                            String.CASE_INSENSITIVE_ORDER.compare(
                                    o1.getComponent().toShortString(),
    ...
    private static void nonDefaultResolution(
            boolean serviceClaimsPaymentAid,
            ServiceAidInfo serviceAidInfo,
        ...
        if (serviceClaimsPaymentAid) {
            ...
            if (VDBG) {
                Log.d(
                        TAG,
                        "nonDefaultResolution: (Ignoring handling service "
    ...
    private static void nonDefaultRouting(
        ...
        if (resolveInfo.services.size() == 1 && makeSingleServiceDefault) {
            if (DBG) {
                Log.d(
                        TAG,
                        "nonDefaultRouting: DECISION: making single handling service "
    ...
    boolean isDefaultOrAssociatedWalletService(ApduServiceInfo serviceInfo, int userId) {
        synchronized (mLock) {
            ...
            if (serviceInfo
                    .getComponent()
                    .getPackageName()
                    .equals(mDefaultWalletHolderPackageName)) {
                return true;
            }
    ...
    /**
     * Resolves a conflict between multiple services handling the same AIDs. Note that the AID
     * itself is not an input to the decision process - the algorithm just looks at the competing
     * services and what preferences the user has indicated. In short, it works like this:
     *
     * <p>1) If there is a preferred foreground service, that service wins 2) Else if there is a
     * default wallet app, that app wins 3) Else, if there is a preferred payment service, that
     * service wins 4) Else, if there is no winner, and all conflicting services will be in the list
     * of resolved services.
     */
    AidResolveInfo resolveAidConflictLocked(
            Collection<ServiceAidInfo> conflictingServices, boolean makeSingleServiceDefault) {
        if (conflictingServices == null || conflictingServices.size() == 0) {
            Log.e(TAG, "resolveAidConflictLocked: No services passed in.");
            return null;
        }
        AidResolveInfo resolveInfo = new AidResolveInfo();
        resolveInfo.category = CardEmulation.CATEGORY_OTHER;
        ...
        if (mPreferredSimType == TelephonyUtils.SIM_TYPE_UNKNOWN) {
            if (DBG) {
                Log.i(
                        TAG,
                        "resolveAidConflictLocked: Sim based service is removed "
                                + "due to unknown sim type");
            }
            filteredServices =
                    conflictingServices.stream()
                            .filter(
                                    serviceAidInfo -> {
                                        if (serviceAidInfo.service.isOnHost()) return true;
                                        String offHost =
                                                serviceAidInfo.service.getOffHostSecureElement();
                                        if (offHost != null) {
                                            return !offHost.startsWith(
        ...
        for (ServiceAidInfo serviceAidInfo : filteredServices) {
            ...
            int userId =
                    UserHandle.getUserHandleForUid(serviceAidInfo.service.getUid()).getIdentifier();
            ...
            if (componentName.equals(mPreferredForegroundService)
                ...
                if (isDefaultOrAssociatedWalletService(serviceAidInfo.service, userId)) {
                    if (VDBG) {
                        Log.d(
                                TAG,
                                "resolveAidConflictLocked: Prioritizing default wallet services");
                    }
                    if (serviceClaimsPaymentAid
                        ...
                        if (VDBG) {
                            Log.d(
                                    TAG,
                                    "resolveAidConflictLocked: Service disabled in default wallet, "
        ...
        if (matchedForeground != null) {
            ...
            if (DBG) {
                Log.d(
                        TAG,
                        "resolveAidConflictLocked: DECISION: routing to foreground preferred "
                                + matchedForeground);
            }
            resolveInfo.defaultService = matchedForeground;

            // Wallet Role Holder and the PreferredPaymentService are mutually exclusive. If the
            // wallet
            ...
            if (DBG) {
                Log.d(
                        TAG,
                        "resolveAidConflictLocked: DECISION: routing to default wallet "
            ...
            if (DBG) {
                Log.d(
                        TAG,
                        "resolveAidConflictLocked: DECISION: routing to payment default "
                                + "default "
                                + matchedPayment);
            }
    ...
    private AidResolveInfo noChildrenAidsPreferred(
        ...
        if (isSubset(aidServices.get(0).aid)) {
            ...
            for (ServiceAidInfo serviceInfo : aidServices) apduServiceList.add(serviceInfo.service);
        ...
        if (aidDefaultInfo.foregroundDefault != null) {
            ...
            if (isSubset(aidServices.get(0).aid)) {
                resolveinfo.prefixInfo =
                        findPrefixConflictForSubsetAid(
                                aidServices.get(0).aid, List.of(resolveinfo.defaultService), true);
            }
            return resolveinfo;
        } else if (mWalletRoleObserver.isWalletRoleFeatureEnabled()) {
            if (!aidDefaultInfo.walletDefaults.isEmpty()) {
                ...
                if (conflictingDefaultInfo.foregroundDefault != null) {
                    ...
                    // Prefix service is default wallet, treat as normal AID conflict for just
                    // prefix
                    if (DBG) {
                        Log.d(
                                TAG,
                                "resolveAidConflictLocked: Default wallet app exists. "
                    ...
                    if (isSubset(aidServices.get(0).aid)) {
                        resolveinfo.prefixInfo =
                                findPrefixConflictForSubsetAid(
                                        ...
                                        List.of(resolveinfo.defaultService),
                                        true);
                    }
                    return resolveinfo;
                }
                ...
                if (conflictingDefaultInfo.foregroundDefault != null
                    ...
                    if (DBG) {
                        Log.d(
                                TAG,
                                "resolveAidConflictLocked: One of the conflicting AID registrations"
    ...
    private int getProfileParentId(int userId) {
        ...
        try {
            ...
            Log.d(TAG, "getProfileParentId: Failed to query parent id for " + userId);
        }
    ...
    void generateServiceMapLocked(List<ApduServiceInfo> services) {
        ...
        UserManager um =
                mContext.createContextAsUser(UserHandle.of(currentUser), /* flags= */ 0)
                        .getSystemService(UserManager.class);

        for (Map.Entry<Integer, List<ApduServiceInfo>> entry : mUserApduServiceInfo.entrySet()) {
            ...
            for (ApduServiceInfo service : entry.getValue()) {
                ...
                for (String aid : service.getAids()) {
                    if (!CardEmulation.isValidAid(aid)) {
                        Log.e(TAG, "generateServiceMapLocked: Aid " + aid + " is not valid.");
                        continue;
                    }
                    if (aid.endsWith("*") && !supportsAidPrefixRegistration()) {
                        Log.e(
                                TAG,
                                "generateServiceMapLocked: Prefix AID "
                                        + aid
                                        + " ignored on device that doesn't support it.");
                        continue;
                    } else if (supportsAidPrefixRegistration()
                            && prefixAids.size() > 0
                            && isExact(aid)) {
                        ...
                        for (String prefixAid : prefixAids) {
                            ...
                            if (aid.startsWith(prefix)) {
                                Log.e(
                                        TAG,
                                        "generateServiceMapLocked: Ignoring exact AID "
                                                + aid
                                                + " because prefix AID "
                                                + prefixAid
                        ...
                        Log.e(
                                TAG,
                                "generateServiceMapLocked: Subset AID "
                                        + aid
                                        + " ignored on device that doesn't support it.");
                        continue;
    ...
    ResolvedPrefixConflictAid findPrefixConflictForSubsetAid(
            String subsetAid, List<ApduServiceInfo> prefixServices, boolean priorityRootAid) {
        ...
        // 2.Find the smallest prefix among matching prefix and add it only if it is not same as
        // susbet AID.
        // 3..If the subset AID and prefix AID are same add only one AID with both prefix , subset
        // bits set.
        // Cut off "#"
                ...
                if (plainSubsetAid.startsWith(plainPrefix)) {
                    if (priorityRootAid) {
                        ...
                        if (CardEmulation.CATEGORY_PAYMENT.equals(
                                        service.getCategoryForAid(prefixAid))
        ...
        if (prefixAids.size() > 0) minPrefix = Collections.min(prefixAids);
    ...
    AidConflicts findConflictsForPrefixLocked(String prefixAid) {
        ...
        if (DBG) {
            Log.d(
                    TAG,
                    "findConflictsForPrefixLocked: Finding AIDs in range ["
                            + plainAid
                            + " - "
                            + lastAidWithPrefix
                            + "]");
        }
        prefixConflicts.conflictMap = mAidServices.subMap(plainAid, true, lastAidWithPrefix, true);
        for (Map.Entry<String, ArrayList<ServiceAidInfo>> entry :
            ...
            if (!entry.getKey().equalsIgnoreCase(prefixAid)) {
                if (DBG)
                    Log.d(
                            TAG,
                            "findConflictsForPrefixLocked: AID "
                                    + entry.getKey()
    ...
    AidConflicts findConflictsForSubsetAidLocked(String subsetAid) {
        ...
        if (DBG) {
            Log.d(
                    TAG,
                    "findConflictsForSubsetAidLocked: Finding AIDs in range ["
                            + firstAid
                            + " - "
                            + lastPlainAid
                            + "]");
        ...
        for (Map.Entry<String, ArrayList<ServiceAidInfo>> entry : mAidServices.entrySet()) {
            String aid = entry.getKey();
            String plainAid = aid;
            if (isSubset(aid) || isPrefix(aid)) plainAid = aid.substring(0, aid.length() - 1);
        ...
        for (Map.Entry<String, ArrayList<ServiceAidInfo>> entry :
            ...
            if (!entry.getKey().equalsIgnoreCase(subsetAid)) {
                if (DBG)
                    Log.d(
                            TAG,
                            "findConflictsForSubsetAidLocked: AID "
                                    + entry.getKey()
                                    + " conflicts with subset AID; "
                                    + " adding handling services for conflict resolution.");
    ...
    void generateAidCacheLocked() {
        ...
        while (!aidsToResolve.isEmpty()) {
            ...
            if (isPrefix(aidToResolve)) {
                ...
                final ArrayList<ServiceAidInfo> prefixServices =
                ...
                // Resolve conflicts
                AidResolveInfo resolveInfo =
                        resolveAidConflictLocked(prefixServices, prefixConflicts.services);
                ...
                if (resolveInfo.defaultService != null) {
                        ...
                        if (prefixConflicts.aids.contains(aid)) {
                            int userId =
                                    UserHandle.getUserHandleForUid(
                                                    resolveInfo.defaultService.getUid())
                                            .getIdentifier();
                            if ((CardEmulation.CATEGORY_PAYMENT.equals(
                                            resolveInfo.defaultService.getCategoryForAid(aid)))
                                    || (resolveInfo
                                                    .defaultService
                                                    .getComponent()
                                                    .equals(mPreferredForegroundService)
                                            && userId == mUserIdPreferredForegroundService)) {
                                AidResolveInfo childResolveInfo =
                                        resolveAidConflictLocked(mAidServices.get(aid), false);
                    ...
                    for (Map.Entry<String, ArrayList<ServiceAidInfo>> entry :
                        ...
                        if (!entry.getKey().equalsIgnoreCase(aidToResolve)) {
                            ...
                            AidResolveInfo childResolveInfo =
                                    resolveAidConflictLocked(entry.getValue(), false);
        ...
        PriorityQueue<String> reversedQueue =
                new PriorityQueue<String>(1, Collections.reverseOrder());
        ...
        while (!reversedQueue.isEmpty()) {
            ...
            if (isSubset(aidToResolve)) {
                if (DBG) {
                    Log.d(
                            TAG,
                            "generateAidCacheLocked: subset resolving aidToResolve  "
                                    + aidToResolve);
                }
                final ArrayList<ServiceAidInfo> subsetServices =
                        new ArrayList<ServiceAidInfo>(mAidServices.get(aidToResolve));
                ...
                // Resolve conflicts
                AidResolveInfo resolveInfo =
                        resolveAidConflictLocked(subsetServices, aidConflicts.services);
        ...
        if (DBG) {
            for (String key : mAidCache.keySet()) {
                Log.d(
                        TAG,
                        "generateAidCacheLocked: aid cache entry"
                                + key
                                + " val:"
    ...
    private int computeAidPowerState(
            boolean isOnHost, boolean requiresScreenOn, boolean requiresUnlock) {
    ...
    public int updateRoutingLocked(boolean force, boolean isOverrideOrRecover) {
        if (!mNfcEnabled) {
            if (DBG) {
                Log.d(
                        TAG,
                        "updateRoutingLocked: Not updating routing table " + "because NFC is off");
            }
        ...
        for (Map.Entry<String, AidResolveInfo> aidEntry : mAidCache.entrySet()) {
            ...
            if (aid.endsWith("*")
                    || (resolveInfo.prefixInfo != null && resolveInfo.prefixInfo.matchingSubset)) {
            ...
            if (resolveInfo.services.isEmpty()) {
                ...
                if (resolveInfo.category.equals(CardEmulation.CATEGORY_PAYMENT)) {
                    aidType.isOnHost = true;
                } else {
        ...
        if (result == AidRoutingManager.CONFIGURE_ROUTING_FAILURE_TABLE_FULL
            ...
            try {
                ...
                Log.e(
                        TAG,
                        "updateRoutingLocked: Error in onLaunchRoutingTableFullDialog: "
                                + exception);
            }
        }
        return result;
    ...
    public void onPreferredPaymentServiceChanged(ComponentNameAndUser service) {
        if (DBG) Log.d(TAG, "onPreferredPaymentServiceChanged: user:" + service.getUserId());
        synchronized (mLock) {
    ...
    public void onPreferredForegroundServiceChanged(ComponentNameAndUser service) {
        if (DBG) Log.d(TAG, "onPreferredForegroundServiceChanged: user:" + service.getUserId());
        synchronized (mLock) {
    ...
    private void generateAssociatedRoleServicesLocked(int userId) {
        ...
        for (ApduServiceInfo service : apduServices) {
            ...
            if (service.wantsRoleHolderPriority()
                    && pm.checkSignatures(
                                    mDefaultWalletHolderPackageName,
                                    service.getComponent().getPackageName())
    ...
    public boolean isPreferredServicePackageNameForUser(String packageName, int userId) {
        if (mPreferredForegroundService != null) {
            if (mPreferredForegroundService.getPackageName().equals(packageName)
                ...
                Log.i(
                        TAG,
                        "isPreferredServicePackageNameForUser: NfcService:"
                                + packageName
                                + "("
                                + userId
    ...
    String dumpEntry(Map.Entry<String, AidResolveInfo> entry) {
        ...
        ComponentName defaultComponent =
                defaultServiceInfo != null ? defaultServiceInfo.getComponent() : null;
        ...
        for (ApduServiceInfo serviceInfo : entry.getValue().services) {
            ...
            sb.append(serviceInfo + " (Description: " + serviceInfo.getDescription() + ")\n");
        }
        return sb.toString();
    }
        ...
        if (Flags.nfcAssociatedRoleServices()) {
            ...
            pw.println(
                    "    Associated role services: "
                            + mAssociatedRoleServices.stream()
                                    .map(service -> service.getComponent().toString())
                                    .toList());
        }
        pw.println("");
