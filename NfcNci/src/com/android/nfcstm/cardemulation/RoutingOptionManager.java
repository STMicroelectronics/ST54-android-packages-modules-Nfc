...
public class RoutingOptionManager {
    ...
    static class SimSettings {
        ...
        SimSettings(int numberOfUicc, int startIndexOfEuicc) {
            ...
            if (numberOfUicc != 0 && numberOfUicc > startIndexOfEuicc) {
                ...
                eUiccIndexs[1] =
                        (numberOfUicc > startIndexOfEuicc + 1)
                                ? startIndexOfEuicc + 1
                                : startIndexOfEuicc;
            }
        }
    ...
    RoutingOptionManager() {
        ...
        mPreferredSimSettings =
                new SimSettings((mOffHostRouteUicc != null) ? mOffHostRouteUicc.length : 0, 1);
    ...
    public void overwriteRoutingTable() {
        ...
        if (mOverrideDefaultRoute != ROUTE_UNKNOWN) {
            if (mOverrideDefaultRoute == ROUTE_DEFAULT) {
                Log.i(
                        TAG,
                        "overwriteRoutingTable: overwrite mDefaultRoute with "
                                + "default config value");
                mDefaultRoute = doGetDefaultRouteDestination();
            } else {
                Log.d(
                        TAG,
                        "overwriteRoutingTable: mDefaultRoute : "
        ...
        if (mOverrideDefaultIsoDepRoute != ROUTE_UNKNOWN) {
            if (mOverrideDefaultIsoDepRoute == ROUTE_DEFAULT) {
                Log.i(
                        TAG,
                        "overwriteRoutingTable: overwrite mDefaultIsoDepRoute "
                                + "with default config value");
                mDefaultIsoDepRoute = doGetDefaultIsoDepRouteDestination();
            } else {
                Log.d(
                        TAG,
                        "overwriteRoutingTable: mDefaultIsoDepRoute : "
        ...
        if (mOverrideDefaultOffHostRoute != ROUTE_UNKNOWN) {
            if (mOverrideDefaultOffHostRoute == ROUTE_DEFAULT) {
                Log.i(
                        TAG,
                        "overwriteRoutingTable: overwrite mDefaultOffHostRoute with "
                                + "default config value");
                mDefaultOffHostRoute = doGetDefaultOffHostRouteDestination();
            } else {
                Log.d(
                        TAG,
                        "overwriteRoutingTable: mDefaultOffHostRoute : "
        ...
        if (mOverrideDefaultScRoute != ROUTE_UNKNOWN) {
            ...
            writeRoutingOption(KEY_DEFAULT_SC_ROUTE, getSecureElementForRoute(mDefaultScRoute));
        }
    ...
    public void recoverOverridedRoutingTable() {
        ...
        mOverrideDefaultRoute =
                mOverrideDefaultIsoDepRoute =
                        mOverrideDefaultOffHostRoute = mOverrideDefaultFelicaRoute = ROUTE_UNKNOWN;
    }
    ...
    public void readRoutingOptionsFromPrefs(
        ...
        if (!mPrefs.contains(KEY_DEFAULT_SC_ROUTE)) {
            writeRoutingOption(KEY_DEFAULT_SC_ROUTE, deviceConfigFacade.getDefaultScRoute());
        }
