...
public class WalletRoleObserver {
    ...
    @VisibleForTesting final OnRoleHoldersChangedListener mOnRoleHoldersChangedListener;
    private Callback mCallback;
    private int mCurrentUser;

    public WalletRoleObserver(
            Context context, RoleManager roleManager, Callback callback, NfcInjector nfcInjector) {
        ...
        this.mOnRoleHoldersChangedListener =
                (roleName, user) -> {
                    ...
                    Context userContext =
                            mContext.createContextAsUser(UserHandle.of(mCurrentUser), 0);
                    ...
                    if (Flags.walletRoleCrossUserEnabled()) {
                        ...
                        if (!Objects.equals(user, UserHandle.of(mCurrentUser))
                                && !Objects.equals(
                                        userManager.getProfileParent(user),
                    ...
                    List<String> roleHolders =
                            roleManager.getRoleHoldersAsUser(RoleManager.ROLE_WALLET, user);
                    String roleHolder = roleHolders.isEmpty() ? null : roleHolders.get(0);
                    if (DBG) {
                        Log.i(
                                TAG,
                                "WalletRoleObserver: Wallet role changed for user "
                                        + user.getIdentifier()
                                        + " to "
                                        + roleHolder);
                    }
        ...
        this.mRoleManager.addOnRoleHoldersChangedListenerAsUser(
                context.getMainExecutor(), mOnRoleHoldersChangedListener, UserHandle.ALL);
    }
    ...
    public PackageAndUser getDefaultWalletRoleHolder(int userId) {
        ...
        try {
            ...
            if (Flags.walletRoleCrossUserEnabled() && crossUserRoleEnabled()) {
                roleUserHandle = userRoleManager.getActiveUserForRole(RoleManager.ROLE_WALLET);
            ...
            List<String> roleHolders =
                    mRoleManager.getRoleHoldersAsUser(RoleManager.ROLE_WALLET, roleUserHandle);
    ...
    public void onUserSwitched(int userId) {
        ...
        if (DBG) {
            Log.i(
                    TAG,
                    "onUserSwitched: Wallet role for user "
                            + userId
                            + ": "
