...
import com.android.nfcstm.DeviceConfigFacade;
import com.android.nfcstm.ExitFrame;
import com.android.nfcstm.ForegroundUtils;
import com.android.nfcstm.NfcEventLog;
import com.android.nfcstm.NfcInjector;
...
public class CardEmulationManager
    ...
    public CardEmulationManager(
            Context context, NfcInjector nfcInjector, DeviceConfigFacade deviceConfigFacade) {
        mContext = context;
        ...
        mWalletRoleObserver =
                new WalletRoleObserver(
                        context, context.getSystemService(RoleManager.class), this, nfcInjector);
        ...
        mPreferredServices =
                new PreferredServices(context, mServiceCache, mAidCache, mWalletRoleObserver, this);
    ...
    private void initialize() {
        if (mPreferredSubscriptionService != null) mPreferredSubscriptionService.initialize();
        ...
        if (mWalletRoleObserver.isWalletRoleFeatureEnabled()) {
            ...
            PackageAndUser roleHolder = mWalletRoleObserver.getDefaultWalletRoleHolder(currentUser);
    ...
    public void onHostCardEmulationActivated(int technology) {
        ...
        if (mDeviceConfigFacade.getIndicateUserActivityForHce() && mPowerManager != null) {
    ...
    public void onServicesUpdated(
            int userId, List<ApduServiceInfo> services, boolean validateInstalled) {
        if (!mWalletRoleObserver.isWalletRoleFeatureEnabled()) {
    ...
    public void onEeListenActivated(boolean isActivated) {
        if (!isActivated) {
            Log.d(
                    TAG,
                    "onEeListenActivated: Listen Mode is deactivated, "
    ...
    void verifyDefaults(int userId, List<ApduServiceInfo> services, boolean validateInstalled) {
        ...
        if (numDefaultPaymentServices > 1) {
            ...
            if (DBG) {
                Log.d(
                        TAG,
                        "verifyDefaults: Current default: "
            ...
            try {
                pm =
                        mContext.createPackageContextAsUser(
                                        "android", /* flags= */ 0, UserHandle.of(userId))
                                .getPackageManager();
    ...
    boolean wasServicePreInstalled(PackageManager packageManager, ComponentName service) {
        try {
            ...
            Log.e(
                    TAG,
                    "wasServicePreInstalled: Service is not currently "
    ...
    boolean isServiceRegistered(int userId, ComponentName service) {
        ...
        if (!serviceFound) {
            ...
            if (DBG) {
                Log.d(
                        TAG,
                        "isServiceRegistered: Didn't find passed in service, "
                                + "invalidating cache");
            }
    ...
    public void onPreferredSubscriptionChanged(int subscriptionId, boolean isActive) {
        ...
        Log.i(
                TAG,
                "onPreferredSubscriptionChanged: subscription_"
                        + subscriptionId
                        + "is active("
                        + isActive
                        + ")"
    ...
    private boolean updateRouteToPreferredSim(Supplier<Integer> getter, Consumer<Integer> setter) {
        ...
        if (se.startsWith(RoutingOptionManager.SE_PREFIX_SIM)) {
            int route =
                    mRoutingOptionManager.getRouteForSecureElement(
    ...
    final class CardEmulationInterface extends INfcCardEmulation.Stub {
        ...
        public boolean isDefaultServiceForCategory(
            ...
            if (mWalletRoleObserver.isWalletRoleFeatureEnabled()) {
                PackageAndUser holder = mWalletRoleObserver.getDefaultWalletRoleHolder(userId);
                if (holder.getPackage() == null) {
                    return false;
                }
                return service.getPackageName().equals(holder.getPackage())
        ...
        @Override
        public boolean setShouldDefaultToObserveModeForService(
                int userId, ComponentName service, boolean enable) {
            NfcPermissions.validateUserId(userId);
            NfcPermissions.enforceUserPermissions(mContext);
            if (!isServiceRegistered(userId, service)) {
                return false;
            }
            Log.d(TAG, "setShouldDefaultToObserveModeForService: (" + service + ") to " + enable);
            boolean currentStatus =
                    mServiceCache.doesServiceShouldDefaultToObserveMode(userId, service);

            if (currentStatus != enable) {
                if (!mServiceCache.setShouldDefaultToObserveModeForService(
        ...
        public boolean registerAidGroupForService(
            ...
            if (!isServiceRegistered(userId, service)) {
                Log.e(
                        ...
                        "registerAidGroupForService: service ("
                                + service
                                + ") isn't registered for user "
                                + userId);
                return false;
            }
            if (!mServiceCache.registerAidGroupForService(
                    userId, Binder.getCallingUid(), service, aidGroup)) {
                return false;
            }
            ...
            mNfcEventLog.logEvent(
                    NfcEventProto.EventType.newBuilder()
                            .setAidRegistration(
                                    NfcEventProto.NfcAidRegistration.newBuilder()
                                            .setAppInfo(
        ...
        public boolean registerPollingLoopFilterForService(
                int userId, ComponentName service, String pollingLoopFilter, boolean autoTransact)
                throws RemoteException {
            NfcPermissions.validateUserId(userId);
            NfcPermissions.enforceUserPermissions(mContext);
            if (!isServiceRegistered(userId, service)) {
                Log.e(
                        TAG,
                        "registerPollingLoopFilterForService: service ("
                                + service
                                + ") isn't registered for user "
                                + userId);
                return false;
            }
            if (!mServiceCache.registerPollingLoopFilterForService(
                    userId, Binder.getCallingUid(), service, pollingLoopFilter, autoTransact)) {
                return false;
            }
            mNfcEventLog.logEvent(
                    NfcEventProto.EventType.newBuilder()
                            .setPollingLoopRegistration(
                                    NfcEventProto.NfcPollingLoopRegistration.newBuilder()
                                            .setAppInfo(
                                                    NfcEventProto.NfcAppInfo.newBuilder()
                                                            .setUid(Binder.getCallingUid())
                                                            .build())
        ...
        @Override
        public boolean removePollingLoopFilterForService(
                int userId, ComponentName service, String pollingLoopFilter)
                throws RemoteException {
            NfcPermissions.validateUserId(userId);
            NfcPermissions.enforceUserPermissions(mContext);
            if (!isServiceRegistered(userId, service)) {
                Log.e(
                        TAG,
                        "removePollingLoopFilterForService: service ("
                                + service
                                + ") isn't registered for user "
                                + userId);
                return false;
            }
            if (!mServiceCache.removePollingLoopFilterForService(
                    userId, Binder.getCallingUid(), service, pollingLoopFilter)) {
                return false;
            }
            mNfcEventLog.logEvent(
                    NfcEventProto.EventType.newBuilder()
        ...
        @Override
        public boolean registerPollingLoopPatternFilterForService(
                int userId,
            ...
            if (!isServiceRegistered(userId, service)) {
                Log.e(
                        TAG,
                        "registerPollingLoopPatternFilterForService: service ("
                                + service
                                + ") isn't registed for user "
                                + userId);
                return false;
            }
            if (!mServiceCache.registerPollingLoopPatternFilterForService(
                    userId,
        ...
        @Override
        public boolean removePollingLoopPatternFilterForService(
            ...
            if (!isServiceRegistered(userId, service)) {
                Log.e(
                        TAG,
                        "removePollingLoopPatternFilterForService: service ("
                                + service
                                + ") isn't registed for user "
                                + userId);
                return false;
            }
            if (!mServiceCache.removePollingLoopPatternFilterForService(
                    userId, Binder.getCallingUid(), service, pollingLoopPatternFilter)) {
                return false;
            }
            mNfcEventLog.logEvent(
                    NfcEventProto.EventType.newBuilder()
        ...
        public AidGroup getAidGroupForService(int userId, ComponentName service, String category)
            ...
            return mServiceCache.getAidGroupForService(
        ...
        public ApduServiceInfo getPreferredPaymentService(int userId) throws RemoteException {
            ...
            return mServiceCache.getService(
                    userId, mAidCache.getPreferredService().getComponentName());
        }

        @Override
        public int setServiceEnabledForCategoryOther(int userId, ComponentName app, boolean status)
                throws RemoteException {
        ...
        public boolean isDefaultPaymentRegistered() throws RemoteException {
            if (mWalletRoleObserver.isWalletRoleFeatureEnabled()) {
                ...
                return mWalletRoleObserver.getDefaultWalletRoleHolder(callingUserId).getPackage()
                        != null;
            }
        ...
        public void overrideRoutingTable(
                int userHandle, String protocol, String technology, String pkg) {
            Log.d(
                    TAG,
                    "overrideRoutingTable: userHandle "
                            + userHandle
                            + ", protocol "
                            + protocol
                            + ", technology "
            ...
            if (android.nfc.Flags.nfcOverrideRecoverRoutingTable()) {
                if (!isPreferredServicePackageNameForUser(
                        pkg, UserHandle.getUserHandleForUid(callingUid).getIdentifier())) {
            ...
            if (!mForegroundUtils.registerUidToBackgroundCallback(
                    mForegroundCallback, callingUid)) {
            ...
            if (DBG) {
                Log.d(
                        TAG,
                        "overrideRoutingTable: protocolRoute "
                                + protocolRoute
                                + ", technologyRoute "
                                + technologyRoute);
            }
        ...
        public void overwriteRoutingTable(
                int userHandle, String aids, String protocol, String technology, String sc) {
            Log.d(
                    TAG,
                    "overwriteRoutingTable(): userHandle: "
                            + userHandle
                            + ", emptyAid: "
                            + aids
                            + ", protocol: "
            ...
            if (DBG) {
                Log.d(
                        TAG,
                        "overwriteRoutingTable(): aidRoute: "
                                + Integer.toHexString(aidRoute)
                                + ", protocolRoute: "
                                + Integer.toHexString(protocolRoute)
                                + ", technologyRoute: "
                                + Integer.toHexString(technologyRoute)
                                + ", scRoute: "
                                + Integer.toHexString(scRoute));
            }
            if (aids != null) {
         ...
         *
         * @throws UnsupportedOperationException if the device does not have required telephony
         *     feature
         */
        ...
        public int setDefaultNfcSubscriptionId(int subscriptionId, String pkgName) {
            ...
            enforceTelephonySubscriptionFeatureWithException(
                    pkgName, "setDefaultNfcSubscriptionId");
        ...
        public int getDefaultNfcSubscriptionId(String pkgName) {
            NfcPermissions.enforceUserPermissions(mContext);
            enforceTelephonySubscriptionFeatureWithException(
                    pkgName, "getDefaultNfcSubscriptionId");
        ...
        @Override
        public void unregisterNfcEventCallback(INfcEventCallback listener) {
            if (!android.nfc.Flags.nfcEventListener()) {
                return;
            }
    ...
    private void callNfcEventCallbacks(ListenerCall call) {
        synchronized (mNfcEventCallbacks) {
            ...
            try {
                IntStream.range(0, numListeners)
                        .forEach(
                                i -> {
                                    try {
    ...
    public void updateForDefaultSwpToEuicc() {
        ...
        if (response.length() >= 4) {
            ...
            if (!TextUtils.equals(statusWord, "9000")) {
                Log.e(
                        TAG,
                        "updateForDefaultSwpToEuicc: ASSIGN_SWP(LSE) Command is fail: "
                                + statusWord);
            }
        } else {
            Log.e(
                    TAG,
                    "updateForDefaultSwpToEuicc: response.length() is wrong length : "
                            + response.length());
        }
    }
        ...
        if (optionalInfo.isPresent()) {
            ...
            if (info.isEmbedded()) {
                return info.getPortIndex() == 0
                        ? TelephonyUtils.SIM_TYPE_EUICC_1
                        : TelephonyUtils.SIM_TYPE_EUICC_2;
            } else {
    ...
    public void updateForShouldDefaultToObserveMode(int userId) {
        ...
        try {
            ...
            boolean enableObserveMode =
                    mServiceCache.doesServiceShouldDefaultToObserveMode(userId, preferredService);
    ...
    public void updateFirmwareExitFramesForWalletRole(int userId) {
        ...
        Stream<ApduServiceInfo> roleServices =
                mServiceCache.getServices(userId).stream()
                        .filter(
                                serviceInfo ->
                                        mAidCache.isDefaultOrAssociatedWalletService(
                                                serviceInfo, userId));
        ...
        Function<String, ExitFrame> toExitFrameOrNull =
                (s) -> {
                    try {
        ...
        // frames
        List<ExitFrame> exitFrames =
                roleServices
                        .flatMap(
                                serviceInfo ->
                                        Stream.concat(
                                                        serviceInfo
                                                                .getPollingLoopFilters()
                                                                .stream(),
                                                        serviceInfo
                                                                .getPollingLoopPatternFilters()
                                                                .stream()
                                                                .map(Pattern::toString))
                                                .filter(serviceInfo::getShouldAutoTransact))
                        .distinct()
