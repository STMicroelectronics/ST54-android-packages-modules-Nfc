...
package com.android.nfcstm.handover;
...
/**
 * Connects / Disconnects from a Bluetooth headset (or any device that might implement BT HSP, HFP,
 * A2DP, or HOGP sink) when touched with NFC.
 *
 * <p>This object is created on an NFC interaction, and determines what sequence of Bluetooth
 * actions to take, and executes them. It is not designed to be re-used after the sequence has
 * completed or timed out. Subsequent NFC interactions should use new objects.
 */
public class BluetoothPeripheralHandover implements BluetoothProfile.ServiceListener {
    ...
    static final String ACTION_TIMEOUT_CONNECT =
            "com.android.nfcstm.handover.action.TIMEOUT_CONNECT";
    static final String ACTION_CANCEL_CONNECT = "com.android.nfcstm.handover.action.CANCEL_CONNECT";
    ...
    public BluetoothPeripheralHandover(
            Context context,
    ...
    public BluetoothPeripheralHandover(
        ...
        mProvisioning =
                Settings.Global.getInt(contentResolver, Settings.Global.DEVICE_PROVISIONED, 0) == 0;
            ...
            case STATE_WAITING_FOR_PROXIES:
                ...
                synchronized (mLock) {
                    if (mA2dp.getConnectedDevices().contains(mDevice)
                            ...
                            || mInput.getConnectedDevices().contains(mDevice)) {
    ...
    void nextStepDisconnect() {
        switch (mState) {
            case STATE_INIT_COMPLETE:
                ...
                synchronized (mLock) {
                    if (mInput.getConnectionState(mDevice) != BluetoothProfile.STATE_DISCONNECTED) {
    ...
    private boolean bluetoothHidHostConnect() {
        // Use reflection to connect to HID device since this method is @hide.
        try {
            Method connectMethod =
                    BluetoothHidHost.class.getDeclaredMethod("connect", BluetoothDevice.class);
            Object ret = connectMethod.invoke(mInput, mDevice);
            return (boolean) ret;
        } catch (ReflectiveOperationException e) {
            Log.d(TAG, "invoke failed" + e);
            return false;
        }
    }
    ...
    void nextStepConnect() {
        switch (mState) {
            ...
            case STATE_BONDING:
                ...
                if (uuids != null) {
                    if (contains(uuids, BluetoothUuid.HID)) {
                        mIsHidDevice = true;
                        Log.d(TAG, "Check HID devices");
                    }
                }
                ...
                synchronized (mLock) {
                    if (mTransport == BluetoothDevice.TRANSPORT_LE || mIsHidDevice) {

                        if (mInput.getConnectionState(mDevice)
                                != BluetoothProfile.STATE_CONNECTED) {
                            mHidResult = RESULT_PENDING;
                            bluetoothHidHostConnect();
    ...
    void startBonding() {
        ...
        if (mOobData != null) {
            if (!mDevice.createBondOutOfBand(
                    mTransport, /* p192 not implemented for LE */ null, mOobData)) {
    ...
    void handleIntent(Intent intent) {
        ...
        if (ACTION_ALLOW_CONNECT.equals(action)) {
            ...
            complete(false);
                ...
                && mState == STATE_BONDING) {
            int bond = intent.getIntExtra(BluetoothDevice.EXTRA_BOND_STATE, BluetoothAdapter.ERROR);
            if (bond == BluetoothDevice.BOND_BONDED) {
                ...
                int reason =
                        intent.getIntExtra(
                                BluetoothDevice.EXTRA_UNBOND_REASON, BluetoothAdapter.ERROR);
                if (mRetryCount < MAX_RETRY_COUNT
        ...
        } else if (BluetoothDevice.ACTION_UUID.equals(action) && mState == STATE_BONDING) {
            nextStepConnect();
    ...
    boolean hasA2dpCapability(ParcelUuid[] uuids, BluetoothClass btClass) {
        if (uuids != null) {
            for (ParcelUuid uuid : uuids) {
                if (uuid.equals(BluetoothUuid.A2DP_SINK)
                        || uuid.equals(BluetoothUuid.ADV_AUDIO_DIST)) {
                    return true;
                }
            }
        }
        ...
        synchronized (mLock) {
            switch (profile) {
                case BluetoothProfile.HEADSET:
                    ...
                    if (mA2dp != null && mInput != null) {
                        Log.d(TAG, "BluetoothProfile.HEADSET onServiceConnected");
                        mHandler.sendEmptyMessage(MSG_NEXT_STEP);
                    }
                    break;
                case BluetoothProfile.A2DP:
                    mA2dp = (BluetoothA2dp) proxy;
                    if (mHeadset != null && mInput != null) {
                        Log.d(TAG, "BluetoothProfile.A2DP onServiceConnected");
                        mHandler.sendEmptyMessage(MSG_NEXT_STEP);
                    }
                    break;
                case BluetoothProfile.HID_HOST:
                    mInput = (BluetoothHidHost) proxy;
                    if (mInput != null && mHeadset != null && mA2dp != null) {
                        Log.d(TAG, "BluetoothProfile.HID_HOST onServiceConnected");
                        mHandler.sendEmptyMessage(MSG_NEXT_STEP);
                    }
                    break;
            }
