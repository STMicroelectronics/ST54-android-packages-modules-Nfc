...
public final class NfcEventLog {
    ...
    public NfcEventLog(
            Context context, NfcInjector nfcInjector, Looper looper, AtomicFile logFile) {
        mContext = context;
    ...
    private void writeListToLogFile() {
        NfcEventProto.EventList.Builder eventListBuilder = NfcEventProto.EventList.newBuilder();
        synchronized (mEventList) {
    ...
    /**
     * Log NFC event Does not block the main NFC thread for logging, posts it to the logging
     * thraead.
     */
    public void logEvent(NfcEventProto.EventType eventType) {
        mHander.post(
                () -> {
                    NfcEventProto.Event event =
                            NfcEventProto.Event.newBuilder()
        ...
        synchronized (mEventList) {
            for (NfcEventProto.Event event : mEventList) {
                ...
                String eventTypeString =
                        event.getEventType()
                                .toString()
                                .replaceAll("# com.android.nfcstm.proto.*", "")
                                .replaceAll("\\s+", " ");
