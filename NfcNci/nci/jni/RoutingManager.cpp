...
namespace android {
extern jboolean t4tNfceeManager_isNdefNfceeEmulationSupported(JNIEnv* e,
                                                              jobject o);
}
...
void RoutingManager::stackCallback(uint8_t event,
  ...
  switch (event) {
    case NFA_CE_REGISTERED_EVT: {
      ...
      routingManager.mCeAidOnDHHandle = eventData->ce_registered.handle;
      SyncEventGuard guard(routingManager.mCeRegisterAiOnDHdEvent);
      routingManager.mCeRegisterAiOnDHdEvent.notifyOne();
    } break;
...
tNFA_TECHNOLOGY_MASK RoutingManager::updateEeTechRouteSetting() {
  ...
  for (uint8_t i = 0; i < localEeInfo.num_ee; i++) {
   ...
   ISO-DEP
    ...
    if ((eeHandle == (mDefaultOffHostRoute | NFA_HANDLE_GROUP_EE)) ||
        (mDefaultOffHostRoute == NFC_DH_ID)) {
      // If HCE is enabled and NFCEE on to support A Mifare/B ISO-DEP
      if (((nfceeFullmask &
            (TECH_A_ISO_DEP | TECH_A_MIFARE | TECH_B_ISO_DEP)) ==
           (TECH_B_ISO_DEP | TECH_A_MIFARE)) ||
          ((nfceeFullmask & (TECH_A_ISO_DEP | TECH_A_MIFARE |
                             TECH_B_ISO_DEP)) == TECH_A_MIFARE)) {
        LOG(DEBUG) << StringPrintf(
  ...
  if (isRfReconfNeeded) {
    ...
    if (android::t4tNfceeManager_isNdefNfceeEmulationSupported(NULL, NULL)) {
      tNFA_EE_MD mode =
          ndefNfceeDisable ? NFC_MODE_DEACTIVATE : NFC_MODE_ACTIVATE;
      SyncEventGuard guard(mEeSetModeEvent);
      NFA_EeModeSet(0x10, mode);
...
*******************************************************************************/
int RoutingManager::
    com_android_nfc_cardemulation_doGetDefaultScRouteDestination(JNIEnv*) {
