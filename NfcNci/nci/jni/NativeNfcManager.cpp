...
const char* gNativeT4tNfceeClassName =
    "com/android/nfcstm/dhimpl/NativeT4tNfceeManager";
    ...
    case NCI_MSG_PROP_ANDROID: {
      ...
      switch (android_sub_opcode) {
        ...
        case NCI_ANDROID_PASSIVE_OBSERVER_SUSPENDED_NTF: {
          ...
          if (param_len <= 2) {
            LOG(ERROR) << "Cannot parse exit frame from "
          ...
          ScopedLocalRef<jobject> dataJavaArray(
              e, e->NewByteArray(exit_frame_len));
          if (dataJavaArray.get() == NULL) {
          ...
          if (exit_frame_len > 0) {
            e->SetByteArrayRegion((jbyteArray)dataJavaArray.get(), 0,
                                  exit_frame_len, (jbyte*)(p_param + 6));
            if (e->ExceptionCheck()) {
              e->ExceptionClear();
...
static jboolean nfcManager_setObserveMode(JNIEnv* e, jobject o,
  ...
  if (isObserveModeSupported(e, o) == JNI_FALSE) {
    LOG(DEBUG)
        << "setObserveMode called when it isn't supported, returning false";
    return false;
  }

  if (isObserveModeSupportedWithoutRfDeactivation(e, o) == JNI_FALSE) {
    LOG(DEBUG)
        << "setObserveMode called when it requires RF off/on, returning false";
    return false;
  }
...
static void nfcManager_enableDiscovery(
    ...
    jboolean reader_mode, jboolean enable_host_routing,
    jbyteArray tech_a_polling_loop_annotation, jboolean restart) {
  if (sIsShuttingDown) return;
...
static void nfcManager_updateSystemCodeRoute(JNIEnv* e, jobject o, jint route) {
  if (sIsShuttingDown) return;
  LOG(DEBUG) << StringPrintf("%s: route=0x%X", __func__, route);
...
static JNINativeMethod gMethods[] = {
    ...
    {"setFirmwareExitFrameTable", "([Lcom/android/nfcstm/ExitFrame;[B)Z",
...
static jbyteArray nfcManager_getProprietaryCaps(JNIEnv* e, jobject o) {
  ...
  if (status == NFA_STATUS_OK) {
    if (!gNfaVsCommand.wait(1000)) {
      LOG(ERROR) << StringPrintf(
          "%s: Timed out waiting for a response to get caps ", __FUNCTION__);
      gVSCmdStatus = NFA_STATUS_FAILED;
    }
  } else {
...
static jboolean nfcManager_setFirmwareExitFrameTable(JNIEnv* env, jobject o,
  ...
  // TODO(b/380455428)
  // Support more than 5 exit frames if firmware allows it. If we do so, might
  // need to send second NCI command if one is too large.
  uint8_t more = 0x00;
  ...
  if (num_exit_frames > 5) {
    LOG(INFO) << "Truncating exit frame table to 5 frames so it fits in a "
                 "single NCI command. "
