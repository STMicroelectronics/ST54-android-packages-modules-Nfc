...
public class NativeNfcTag implements TagEndpoint {
    ...
    /**
     * connectWithIdx() is use by findAndReadNdef() Update of variables mConnectedTechIndex and
     * mConnectedHandle when performing reconnect().
     */
                ...
                switch (mTechList[i]) {
                    case TagTechnology.NFC_A:
                        if ((mTechActBytes[i] != null) && (mTechActBytes[i].length > 0)) {
                            extras.putShort(
                                    NfcA.EXTRA_SAK, (short) (mTechActBytes[i][0] & (short) 0xFF));
                        } else {
